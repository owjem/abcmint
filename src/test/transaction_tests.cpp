...
BOOST_AUTO_TEST_CASE(tx_valid)
{
    ...
    map<vector<unsigned char>, vector<unsigned char>> mapPubKey;
            for (unsigned int i = 0; i < tx.vin.size(); i++)
            {
                ...
                BOOST_CHECK_MESSAGE(VerifyScript(tx.vin[i].scriptSig, mapprevOutScriptPubKeys[tx.vin[i].prevout], tx, i, test[2].get_bool() ? SCRIPT_VERIFY_P2SH : SCRIPT_VERIFY_NONE, 0, mapPubKey), strTest);
            }
        }
    }
}
...
BOOST_AUTO_TEST_CASE(tx_invalid)
{
    ...
    map<vector<unsigned char>, vector<unsigned char>> mapPubKey;
            for (unsigned int i = 0; i < tx.vin.size() && fValid; i++)
            {
                ...
                fValid = VerifyScript(tx.vin[i].scriptSig, mapprevOutScriptPubKeys[tx.vin[i].prevout], tx, i, test[2].get_bool() ? SCRIPT_VERIFY_P2SH : SCRIPT_VERIFY_NONE, 0, mapPubKey);
            }
