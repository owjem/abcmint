...
template <class Type>
size_t cudaData<Type>::size_host() { return sz_host; }

template <class Type>
size_t cudaData<Type>::size_dev() { return sz_dev; }

template <class Type>
void cudaData<Type>::clear() { memset(host, 0, sz_host); }
...
uint32_t* check_thread(uint32_t* deg2, uint32_t* deg1, uint32_t thread,
    uint32_t N)
{
    uint32_t rounds;
    uint32_t x = 0;
...
extern uint32_t* check_thread(uint32_t* deg2, uint32_t* deg1, uint32_t thread,
    uint32_t N);
