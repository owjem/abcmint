...
bool AddOrphanTx(const CTransaction& tx, NodeId peer)
{
    ...
    if (sz > 5000)
    {
        LogPrint("mempool", "[MEMPOOL] ignoring large orphan tx (size: %u, hash: %s)\n", sz, hash.ToString());
        return false;
    }
    ...
    LogPrint("mempool", "[MEMPOOL] stored orphan tx %s (mapsz %u prevsz %u)\n", hash.ToString(),
...
void EraseOrphansFor(NodeId peer)
{
    ...
    if (nErased > 0) LogPrint("mempool", "[MEMPOOL] Erased %d orphan tx from peer %d\n", nErased, peer);
}
                        ...
                        bool* pfMissingInputs, bool fRejectInsaneFee)
{
...

    {
        ...
        if (fLimitFree && nFees < CTransaction::nMinRelayTxFee)
        {
            ...
            LogPrint("mempool", "[MEMPOOL] Rate limit dFreeCount: %g => %g\n", dFreeCount, dFreeCount+nSize);
            dFreeCount += nSize;
        }
    ...
    LogPrint("mempool", "[MEMPOOL] AcceptToMemoryPool: : accepted %s (poolsz %u )\n",
             hash.ToString(),
             pool.mapTx.size());
    return true;
}
...
void CombinePubkey(map<vector<unsigned char>, vector<unsigned char>>&  mapPubKey, const CTransaction& tx) {
    ...
    LogPrint("pk", "[PK] mapPubKey  size(%ld) \n",  mapPubKey.size());
...
bool FindPubkeyFromScript(const CScript& script,const unsigned int nHeight,const unsigned int noffset, const unsigned int noffsetVin) {
    ...
    // LogPrint("pk", "[PK] [%s] noffset[%ld] noffsetVin[%ld] offset[%i] \n", __func__, noffset, noffsetVin, offset);
    // LogPrint("pk", "[PK] [%s] pos[%s][%ld][%ld] \n", __func__, HexStr(pos.Raw()), pos.GetHeight(), pos.GetPubKeyOffset());
                }
            }
        }
    }
...
const int64_t blockValue[45] = { 250000000, 5, 10, 20, 40, 40, 40, 40, 40, 40, 40, 160, 160, 160, 160, 160, 160, 640, 640, 1280, 1280, 2563, 2560, 1884, 1386, 1020, 751, 552, 406, 299, 220, 162, 119, 87, 64, 47, 35, 25, 18, 13, 10, 7, 5, 4, 184230000000 };
...
static const int64_t nNewTargetTimespan = 3.5 * 24 * 60 * 60; // 3.5 days
static const int64_t nTargetTimespan = 14 * 24 * 60 * 60; // two weeks
static const int64_t nTargetSpacing = 10 * 60;
static const int64_t nInterval = nTargetTimespan / nTargetSpacing;
static const int64_t nNewInterval = nNewTargetTimespan / nTargetSpacing;
...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    int64_t _interval = nNewInterval;
    int64_t _targetTimespan = nNewTargetTimespan;
    // Only change once per interval
    if (pindexLast->nHeight < 22176) {
        _interval = nInterval;
        _targetTimespan = nTargetTimespan;
    }

    if ((pindexLast->nHeight+1) % _interval != 0)
    {
        if (TestNet())
        {
            ...
            else
            {
                ...
                while (pindex->pprev && pindex->nHeight % _interval != 0 && pindex->nBits == nProofOfWorkLimit)
                    pindex = pindex->pprev;
                return pindex->nBits;
            }
        }
    ...
    for (int i = 0; pindexFirst && i < _interval - 1; i++)
    ...
    if (nActualTimespan < _targetTimespan / 4)
        nActualTimespan = _targetTimespan / 4;
    if (nActualTimespan > _targetTimespan * 4)
        nActualTimespan = _targetTimespan * 4;

    // Retarget
    int64_t nAverageTime= nActualTimespan / _interval;

    int64_t nAddVariables = (int64_t)std::llround(std::log2(((long double)nTargetSpacing) / nAverageTime));
    int64_t bnNew = pindexLast->nBits + nAddVariables;
    ...
    LogPrintf("After:  %08x  %s \n", (unsigned int &)bnNew, CBigNum().SetCompact(bnNew).getuint256().ToString());
...
void static InvalidChainFound(CBlockIndex* pindexNew)
{
    ...
    LogPrintf("InvalidChainFound: invalid block=%s  height=%d  log2_work=%.8g  date=%s\n",
      ...
      log(pindexNew->nChainWork)/log(2.0), DateTimeStrFormat("%Y-%m-%d %H:%M:%S",
      pindexNew->GetBlockTime()));
    LogPrintf("InvalidChainFound:  current best=%s  height=%d  log2_work=%.8g  date=%s\n",
      chainActive.Tip()->GetBlockHash().ToString(), chainActive.Height(), log(chainActive.Tip()->nChainWork)/log(2.0),
...
bool ConnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    if (block.GetHash() == Params().HashGenesisBlock()) {
        ...
        {
            const CTransaction &tx = block.vtx[i];
            assert(view.SetCoins(block.GetTxHash(i), CCoins(tx, pindex->nHeight)));
        }
        return true;
    }
...
void static UpdateTip(CBlockIndex *pindexNew) {
    ...
    LogPrintf("UpdateTip: new best=%s  height=%d  log2_work=%.8g  tx=%lu  date=%s progress=%f\n",
      chainActive.Tip()->GetBlockHash().ToString(), chainActive.Height(), log(chainActive.Tip()->nChainWork)/log(2.0), (unsigned long)chainActive.Tip()->nChainTx,
      DateTimeStrFormat("%Y-%m-%d %H:%M:%S", chainActive.Tip()->GetBlockTime()),
    ...
    if (!fKnown) {
        ...
        if (nNewChunks > nOldChunks) {
            if (CheckDiskSpace(nNewChunks * BLOCKFILE_CHUNK_SIZE - pos.nPos)) {
                ...
                if (file) {
                    LogPrintf("Pre-allocating up to position 0x%x in blk%08u.dat\n", nNewChunks * BLOCKFILE_CHUNK_SIZE, pos.nFile);
...
bool FindUndoPos(CValidationState &state, int nFile, CDiskBlockPos &pos, unsigned int nAddSize)
{
    ...
    if (nNewChunks > nOldChunks) {
        if (CheckDiskSpace(nNewChunks * UNDOFILE_CHUNK_SIZE - pos.nPos)) {
            ...
            if (file) {
                LogPrintf("Pre-allocating up to position 0x%x in rev%08u.dat\n", nNewChunks * UNDOFILE_CHUNK_SIZE, pos.nFile);
...
void PrintBlockTree()
{
    ...
    while (!vStack.empty())
    {
        ...
        LogPrintf("%d (blk%08u.dat:0x%x)  %s  tx %u\n",
            pindex->nHeight,
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    else if (strCommand == "inv")
    {
        ...
        for (unsigned int nInv = 0; nInv < vInv.size(); nInv++)
        {
            ...
            LogPrint("net", "[NET] got inventory: %s  %s\n", inv.ToString(), fAlreadyHave ? "have" : "new");
    ...
    else if (strCommand == "getdata")
    {
        ...
        if (fDebug || (vInv.size() != 1))
            LogPrint("net", "[NET] received getdata (%u invsz)\n", vInv.size());

        if ((fDebug && vInv.size() > 0) || (vInv.size() == 1))
            LogPrint("net", "[NET] received getdata for: %s\n", vInv[0].ToString());
    ...
    else if (strCommand == "getblocks")
    {
        ...
        if (pindex)
            pindex = chainActive.Next(pindex);
        int nLimit = 500;
        LogPrint("net", "[NET] getblocks %d to %s limit %d\n", (pindex ? pindex->nHeight : -1), hashStop.ToString(), nLimit);
        for (; pindex; pindex = chainActive.Next(pindex))
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                LogPrint("net", "[NET] getblocks stopping at %d %s\n", pindex->nHeight, pindex->GetBlockHash().ToString());
                break;
            }
            ...
            if (--nLimit <= 0)
            {
                ...
                LogPrint("net", "[NET] getblocks stopping at limit %d %s\n", pindex->nHeight, pindex->GetBlockHash().ToString());
    ...
    else if (strCommand == "getheaders")
    {
        ...
        int nLimit = 2000;
        LogPrint("net", "[NET] getheaders %d to %s\n", (pindex ? pindex->nHeight : -1), hashStop.ToString());
        for (; pindex; pindex = chainActive.Next(pindex))
        {
    ...
    else if (strCommand == "tx")
    {
        ...
        if (AcceptToMemoryPool(mempool, state, tx, true, &fMissingInputs))
        {
            ...
            LogPrint("mempool", "[MEMPOOL] AcceptToMemoryPool: %s %s : accepted %s (poolsz %u)\n",
            ...
            for (unsigned int i = 0; i < vWorkQueue.size(); i++)
            {
                     ...
                     ++mi)
                {
                    ...
                    {
                        LogPrint("mempool", "[MEMPOOL] accepted orphan tx %s\n", orphanHash.ToString());
                    ...
                    else if (!fMissingInputs2)
                    {
                        ...
                        if (stateDummy.IsInvalid(nDos) && nDos > 0)
                        {
                            ...
                            LogPrint("mempool", "[MEMPOOL] invalid orphan tx %s\n", orphanHash.ToString());
                        }
                        // too-little-fee orphan
                        LogPrint("mempool", "[MEMPOOL] removed orphan tx %s\n", orphanHash.ToString());
                    }
                    mempool.check(pcoinsTip);
                }
            }
        ...
        else if (fMissingInputs)
        {
            ...
            if (nEvicted > 0)
                LogPrint("mempool", "[MEMPOOL] mapOrphan overflow, removed %u tx\n", nEvicted);
        }
        int nDoS = 0;
        if (state.IsInvalid(nDoS))
        {
            LogPrint("mempool", "[MEMPOOL] %s from %s %s was not accepted into the memory pool: %s\n", tx.GetHash().ToString(),
                pfrom->addr.ToString(), pfrom->cleanSubVer,
    ...
    else if (strCommand == "block" && !fImporting && !fReindex) // Ignore blocks received while importing
    {
        ...
        LogPrint("net", "[NET] received block %s\n", block.GetHash().ToString());
        // block.print();
    ...
    else if (strCommand == "pong")
    {
        ...
        if (!(sProblem.empty())) {
            LogPrint("net", "[NET] pong %s %s: %s, %x expected, %x received, %u bytes\n",
                pfrom->addr.ToString(),
                pfrom->cleanSubVer,
                sProblem,
...
{
    {
        ...
        while (!pto->fDisconnect && state.nBlocksToDownload && state.nBlocksInFlight < MAX_BLOCKS_IN_TRANSIT_PER_PEER) {
            ...
            LogPrint("net", "[NET] Requesting block %s from %s\n", hash.ToString().c_str(), state.name.c_str());
        ...
        while (!pto->fDisconnect && !pto->mapAskFor.empty() && (*pto->mapAskFor.begin()).first <= nNow)
        {
            ...
            {
                if (fDebug)
                    LogPrint("net", "[NET] sending getdata: %s\n", inv.ToString());
