...
Value signrawtransaction(const Array& params, bool fHelp)
{
    ...
    for (unsigned int i = 0; i < mergedTx.vin.size(); i++)
    {
        ...
        if (!fHashSingle || (i < mergedTx.vout.size()))
            SignSignature(mapPubKey, keystore, prevPubKey, mergedTx, i, nHashType);
    ...
    map<vector<unsigned char>, vector<unsigned char>> mapPubKey;
        if (!VerifyScript(txin.scriptSig, prevPubKey, mergedTx, i, SCRIPT_VERIFY_P2SH | SCRIPT_VERIFY_STRICTENC, 0, mapPubKey))
            fComplete = false;
    }
