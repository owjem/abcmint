...
/* compress 1024-bits */
static int  sha512Compress(Sha512 * md, unsigned char *buf) {
...
int sha512Process (Sha512 * md, const unsigned char *in, unsigned long inlen){                                                                                           \
    ...
    while (inlen > 0) {                                                                   
        if (md->curlen == 0 && inlen >= 128) {                        
           if ((err = sha512Compress(md, const_cast<unsigned char *>(in))) != PQCRYPT_OK) {             
              return err;                                                               
           }                                                                           
           md->length     += 128 * 8;                                      
           in             += 128;                                                 
           ...
           inlen          -= n;                                                           
           if (md->curlen == 128) {                                    
              if ((err = sha512Compress(md, md->buf)) != PQCRYPT_OK) {            
                 return err;                                                                
              }                                                                             
              md->length += 8*128;                                       
              md->curlen = 0;                                                   
...
/**
   ...
   @return PQCRYPT_OK if successful
*/
int sha512Done(Sha512 * md, unsigned char *out) {
    int i;
...
int pqcSha512(const unsigned char * in, unsigned long inlen, unsigned char * out) {
	  Sha512 md;
	  int status;
      status = sha512Init(&md);
      status = sha512Process(&md, in, inlen);
      status = sha512Done(&md, out);
	  return status;
}
