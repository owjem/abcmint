...
std::string HelpMessage(HelpMessageMode hmm)
{
    ...
    strUsage += "  -port=<port>           " + _("Listen for connections on <port> (default: 8888 or testnet: 18888)") + "\n";
    ...
    strUsage += "  -useddefaultkey        " + _("used defaultkey for miner") + "\n";
...
void ThreadImport(std::vector<boost::filesystem::path> vImportFiles)
{
    ...
    if (fReindex) {
        ...
        while (true) {
            ...
            if (!file)
                break;
            LogPrintf("Reindexing block file blk%08u.dat...\n", (unsigned int)nFile);
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    fUsedDefaultKey = GetBoolArg("-useddefaultkey", true);
#endif
    ...
    if (!filesystem::exists(blocksDir))
    {
        ...
        for (unsigned int i = 1; i < 10000; i++) {
            filesystem::path source = GetDataDir() / strprintf("blk%05u.dat", i);
            if (!filesystem::exists(source)) break;
            filesystem::path dest = blocksDir / strprintf("blk%08u.dat", i-1);
            try {
                ...
                LogPrintf("Error hardlinking blk%05u.dat : %s\n", i, e.what());
                break;
            }
        }
        if (linked)
